...
namespace Cubix.Filters
{
    ...
    public class AdminTokenFilter : IActionFilter
    {
        ...
        ///// Constructor
        ///// </summary>
        public AdminTokenFilter(IDistributedCache distributedCache)
        {
        ...
        public void OnActionExecuting(ActionExecutingContext context)
        {
            if (context.HttpContext.Request.Headers.ContainsKey("Authorization"))
            {
                ...
                TokenCacheModel oTokenCacheModel = Cache.GetToken(_distributedCache, Constants.TOKEN_PREFIX_ADMIN_USER + LoginId);
                //Match the token 
                if (oTokenCacheModel.UserTypeId != Constants.ADMIN_USER || oTokenCacheModel.Token != token)
                {
                    ErrorModel oResultModel = new ErrorModel(Constants.INSUFFIECIENT_PRIVILEDGE, Constants.INSUFFIECIENT_PRIVILEDGE_MESSAGE);
                    context.Result = new JsonResult(oResultModel);
                }
